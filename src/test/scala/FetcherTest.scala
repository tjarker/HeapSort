...
class FetcherTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "create the right read requests" in {
    test(new Fetcher(HeapParameters(32,4,8))).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      ...
      dut.io.res.children.zip(children).foreach { case (p,c) => p.item.expect(c.U) }
      dut.io.res.children.zip(childrenIndex).foreach { case (p,c) => p.index.expect(c.U) }
